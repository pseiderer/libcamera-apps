...
using RequestCompletePayload = LibcameraHdr::RequestCompletePayload;
...
static void event_loop(LibcameraHdr &app)
{
	...
	for (unsigned int count = 0; ; count++)
	{
		...
		ControlList &metadata = std::get<RequestCompletePayload>(msg.payload).metadata;
		...
		else if (count == PREVIEW_FRAMES)
		{
			...
			app.GetFrameInfo(metadata, &exposure_time, &ag, &dg, colour_gains);
			app.StopCamera();
			...
			ControlList controls;
			controls.set(controls::ExposureTime, (int32_t)(exposure_time * exp_adjust));
			controls.set(controls::AnalogueGain, ag * dg);
			controls.set(controls::ColourGains, { colour_gains[0], colour_gains[1] });
			app.SetControls(controls);
			app.StartCamera();
		}
		else
		{
			...
			app.ShowPreview(buffers, stream);
		}
	}
}
